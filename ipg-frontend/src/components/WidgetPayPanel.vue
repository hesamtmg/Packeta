...
const emit = defineEmits<{
  complete: [result: { transactionId: string; status: 'COMPLETED' | 'FAILED' }];
}>();
...
// attemptSettlement. Only the credit-shortfall branch ever navigates away
// (real ZarinPal, can't be rendered inline).
type Step =
  | 'loading'
  | 'phone'
  | 'otp'
...
async function onVerifyOtp() {
  ...
  try {
    const result = await packetaRequest<{ sessionToken: string; wallets: EligibleWallet[] }>(
      ...
      { method: 'POST', body: { authority: props.authority, code: otpCode.value } },
    );
...
// Masks a wallet id the same cosmetic way WidgetAccountPanel.vue does.
...
async function onContinueWithWallet() {
  ...
  try {
    const result = await packetaRequest<{
      ...
      method: 'POST',
      body: { authority: props.authority, sessionToken: sessionToken.value, walletId: selectedWalletId.value },
    });
...
// The happy path never navigates anywhere: confirm flips the sandbox IPG's
...
// tells the embedding page via a `complete` emit instead of navigating.
async function attemptSettlement() {
  step.value = 'confirming';
  try {
    await apiRequest(`/payments/${props.authority}/confirm`, { method: 'POST' });
...
// payment, which cannot render inline. window.top resolves to window
// itself when this panel isn't framed at all (the normal native-mount
// case), and still correctly breaks out if someone manually iframes the
// routed fallback page instead. ZarinPal's callback is already hardcoded
// server-side to the full-page /pay/:authority route, which already knows
// how to finish and hand the browser back to the merchant via the wallet's
// own callbackUrl.
...
onMounted(async () => {
  try {
    chargeInfo.value = await packetaRequest<ChargeStatus>(
      `/purchase-gateway/charge/${props.authority}/status`,
    );
  } catch (err) {
  ...
  // Setting the local `locale` ref (rather than the old
  ...
  locale.value = chargeInfo.value.language === 'fa' ? 'fa' : 'en';
