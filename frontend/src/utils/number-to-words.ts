...
// English or Farsi — used as a small caption under an amount ("1,200,000"
// -> "one million two hundred thousand" / "یک میلیون و دویست هزار") so a
// long digit string is easier to read at a glance, especially for Rial
// amounts with many digits.
...
function integerToEnglishWords(n: number): string {
  ...
  while (n > 0) {
    const group = n % 1000;
    if (group > 0) {
      groups.unshift(`${enHundreds(group)}${EN_SCALES[scale] ? ` ${EN_SCALES[scale]}` : ''}`);
    }
    n = Math.floor(n / 1000);
    scale += 1;
  }
...
// a currency with 2 decimal places) — the same number formatAmount() renders.
