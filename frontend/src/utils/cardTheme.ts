// Single source of truth for a wallet-type's card gradient: the preset
// palette an admin picks from (AdminWalletTypesView's swatch picker) and
// the fallback used when a type hasn't had a color chosen yet (Dashboard-
// View's wallet cards). Keep the `key`s in sync with the backend's
// CARD_COLOR_PRESETS (backend/src/wallet-types/entities/wallet-type.entity.ts)
// — that list exists only to validate against this one.
